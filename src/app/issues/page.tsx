...
// Keep this in sync with app/page.tsx and app/issues/new/page.tsx.
// If you've since moved font loading into app/layout.tsx, delete this
// block and the className below.
    ...
    <div className={`${manrope.className} min-h-screen text-slate-100`}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-14">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-10">
          ...
          <Link
            href="/issues/new"
            className="px-4 py-2.5 bg-gradient-to-r from-[#6E9765] to-[#45613F] hover:from-[#7CA773] hover:to-[#3B5336] text-white font-semibold rounded-xl transition-all flex items-center gap-2 shadow-lg shadow-[#6E9765]/20 text-sm active:scale-95"
          >
            <Plus className="w-4 h-4" /> Гомдол гаргах
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-24 bg-slate-900/60 backdrop-blur-sm rounded-2xl border border-slate-800">
            ...
            <Link
              href="/issues/new"
              className="text-[#8CB183] font-medium hover:text-[#A9C4A0] hover:underline text-sm"
            >
              Анхны гомдлыг мэдээлэх &rarr;
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {issues.map((issue) => (
              <div
                key={issue.id}
                className="bg-slate-900/60 backdrop-blur-sm rounded-xl border border-slate-800 p-5 shadow-lg shadow-black/10 flex flex-col justify-between transition-colors hover:border-slate-700"
              >
                <div>
                  <div className="flex items-start justify-between gap-3 mb-3">
                  ...
                  {issue.imageUrl && (
                    <div className="mb-4 overflow-hidden rounded-lg h-40 bg-slate-950/60 border border-slate-800/60">
